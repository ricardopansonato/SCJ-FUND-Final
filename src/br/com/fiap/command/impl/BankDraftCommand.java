...
public class BankDraftCommand extends Command {

	public BankDraftCommand(TelegramBot bot, Update update, Long chatId) {
		...
		if (!user.isEmpty()) {
			...
			if (data.getRight().size() > 0) {
				...
				data.getValue().forEach(a -> {
					if(a.getType().equals(AccountStatementType.WITHDRAW)) {
				...
				sb.append("Não foi realizado nenhum saque\n");
				SendMessage request = new SendMessage(chatId,  sb.toString()).parseMode(ParseMode.Markdown);
				bot.execute(request);
			}
		} else {
