...
public class CommandFactory {
	public Command createCommand(TelegramBot bot, Update update, Long chatId) {
		if (update.message() != null) {
			if (update.message().text().startsWith("/start")) {
				return new WelcomeCommand(bot, update, chatId);
			} else if (update.message().text().startsWith("/help")) {
				return new HelpCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/create")) { 
			    return new CreateAccountCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/show")) { 
			    return new ShowAccountCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/deposit")) { 
			    return new DepositCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/withdraw")) { 
			    return new WithdrawCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/modify")) { 
			    return new ModifyAccountCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/statements")) { 
			    return new AccountStatementsCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/dependent")) { 
			    return new CreateDependentAccountCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/loan")) { 
			    return new LoanCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/list_loan")) { 
			    return new ListLoanCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/bank_draft")) { 
			    return new BankDraftCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/bank_fees")) { 
			    return new BankFeesCommand(bot, update, chatId);
		    } else if (update.message().text().startsWith("/bank_posting")) { 
			    return new BankPostingCommand(bot, update, chatId);
		    }
		}
		return new HelpCommand(bot, update, chatId);
